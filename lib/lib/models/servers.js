...
const servers = {
	 ...
	 * @param port
	 * @param mockApi
	 * @param {boolean} forceAdd Will add a server even though a server with same host/port exists.
	 * @returns {*}
	 */
	add: function({ name, host, port, mockApi, forceAdd = false }) {
		...
		let serverId = forceAdd ? uuid() : serverPropsToName({ host, port });
		if(servers.exists({ id: serverId })) {
			return SERVERS[serverId]
		} else {
			SERVERS[serverId] = new MopidyServer({ host, port, name, serverId, mockApi });
			return SERVERS[serverId];
		}
	},
